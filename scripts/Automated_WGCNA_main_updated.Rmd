...
knitr::opts_knit$set(root.dir = "/Users/abdelazizawad/AML_project/WGCNA")
setwd("/Users/abdelazizawad/AML_project/WGCNA")
getwd()
```
...
metadata_binary <- model.matrix(~ 0 + three_year_survival, data = metadata)
colnames(metadata_binary)
colnames(metadata_binary) <- gsub("three_year_survival", "", colnames(metadata_binary))
colnames(metadata_binary) <- c("alive", "dead")
...
Run_WGCNA= function(data, metadata_binary, power,minModuleSize, hub_threshold ,GS_threshold, networkType,
  ...
  write.csv(module_eigengenes, "results/module_eigengenes.csv")
  ...
  write.csv(module.gene.assign, "results/gene_module_assignment.csv")
  ...
  modules= df[-1,]
  write.csv(modules, paste0("results/", "modules_gene_id.csv"), row.names = FALSE)
  ...
  write.csv(gene.signf.corr, "results/gene.trait.corr.csv")
  ...
  write.csv(datKME, "results/module_membership.csv")
  ...
  hubs= df[-1,]
  write.csv(hubs, paste0("results/", "module_hubs.csv"), row.names = FALSE)
  ...
  names(df) = paste0(unlist(df[1,]), "_hub") 
  hubs= df[-1,]
  write.csv(hubs, paste0("results/", "phenotype_cor_hubs.csv"), row.names = FALSE)
  ...
  write.csv(df[-1,], "results/modules_gene_name.csv", row.names = F)
  ...
  for (j in seq_along(combined_hubs[[i]])) {
    ...
    write.csv(mapped_hubs, paste0("results/", names(combined_hubs)[i],"_hubs_names.csv"), row.names = FALSE)
  ...
  for( i in   module_names) {
    ...
    cyt = exportNetworkToCytoscape(TOM_mod,
                                   edgeFile = paste("results/CytoscapeInput-edges-.02tom", paste(i, collapse="-"), ".txt", sep=""),
